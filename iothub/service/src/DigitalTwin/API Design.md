...
### These APIs are supported on .NET 472, NS 2.0, NS 2.1.
...
If we want to have the ttl customizable, we can take it is as an optional param: (NOT IMPLMENTING THIS RIGHT NOW - will add if there is a feature request)
```csharp
...
- We will provide a basic digital twin type for the `GetAsync<T>`, similar to ADT.
```json
{
  "$dtId": "tc",
