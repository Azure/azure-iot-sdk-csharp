    ...
    public class JsonSerializerSettings
    {
        ...
        private static void IgnoreEmptyCollectionsAndUnsetEtags(JsonTypeInfo typeInfo)
        {
            ...
            foreach (var prop in typeInfo.Properties)
            {
                ...
                else if (typeof(ETag).IsAssignableFrom(prop.PropertyType))
                {
                    // Only serialize the ETag if it was set to some non-default value
                    prop.ShouldSerialize = (_, value) =>
                    {
                        if (value == null || value == default || value.Equals(new ETag()))
                        {
                            return false;
                        }
