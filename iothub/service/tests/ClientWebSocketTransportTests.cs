...
namespace Microsoft.Azure.Devices.Api.Test
{
    ...
    public class ClientWebSocketTransportTests
    {
...
#if NET451
        private static LegacyClientWebSocketTransport s_legacyClientWebSocketTransport;
#endif
        ...
        public async Task LegacyWebSocketReadWriteTest()
        {
            ...
            s_legacyClientWebSocketTransport = new LegacyClientWebSocketTransport(websocket, TimeSpan.FromSeconds(60), null, null);
            ...
            // Once Read operation is complete, close websocket transport
            s_legacyClientWebSocketTransport.CloseAsync(TimeSpan.FromSeconds(30)).Wait(CancellationToken.None);
        }
        ...
        public async Task LegacyWebSocketReadAfterCloseTest()
        {
            var websocket = new IotHubClientWebSocket(WebSocketConstants.SubProtocols.Amqpwsb10);
            await websocket.ConnectAsync(IotHubName, Port, "ws://", TimeSpan.FromMinutes(1)).ConfigureAwait(false);
            s_legacyClientWebSocketTransport = new LegacyClientWebSocketTransport(websocket, TimeSpan.FromMinutes(1), null, null);
            await s_legacyClientWebSocketTransport.CloseAsync(TimeSpan.FromSeconds(30)).ConfigureAwait(false);
            var args = new TransportAsyncCallbackArgs();
            var byteArray = new byte[10];
            args.SetBuffer(byteArray, 0, 10);
