...
rem -----------------------------------------------------------------------------
rem -- build csharp E2E
rem -----------------------------------------------------------------------------

call nuget restore -config "%current-path%\NuGet.Config" "%build-root%\e2e\iothub_csharp_e2e.sln"
if %build-clean%==1 (
    call :clean-a-solution "%build-root%\e2e\iothub_csharp_e2e.sln" %build-config% %build-platform%
    if not !errorlevel!==0 exit /b !errorlevel!
)
call :build-a-solution "%build-root%\e2e\iothub_csharp_e2e.sln" %build-config% %build-platform%
if not !errorlevel!==0 exit /b !errorlevel!
