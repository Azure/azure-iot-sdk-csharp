    ...
    internal sealed class MqttTransportHandler : TransportHandler, IMqttIotHubEventHandler
    {
        ...
        private async Task<Message> SendTwinRequestAsync(Message request, string rid, CancellationToken cancellationToken)
        {
                    ...
                    if (ParseResponseTopic(possibleResponse.MqttTopicName, out string receivedRid, out int status))
                    {
                        if (rid == receivedRid)
                        {
                            if (status >= 300)
                            {
                                ...
                                if (status == 429)
                                {
                                    throw new IotHubClientException($"Request {rid} was throttled by the server", IotHubErrorCode.Throttled);
                                }
                                else
                                {
            ...
            try
            {
                ...
                {
                    throw new IotHubClientException($"Response for message {rid} not received", IotHubErrorCode.Timeout);
                }
            ...
            IotHubClientMqttSettings settings)
        {
            return async (addresses, port) =>
            {
                ...
                return channel ?? throw new IotHubClientException("MQTT channel open failed.", null, IotHubErrorCode.NetworkErrors);
            };
        }
        ...
        private void EnsureValidState(bool throwIfNotOpen = true)
        {
            ...
            {
                throw new IotHubClientException("MQTT connection is not established. Please retry later.", null, IotHubErrorCode.NetworkErrors);
            }
        }
