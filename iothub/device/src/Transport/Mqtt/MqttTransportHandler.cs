...
namespace Microsoft.Azure.Devices.Client.Transport.Mqtt
{
    internal sealed class MqttTransportHandler : TransportHandler, IMqttIotHubEventHandler
    {
        ...
        // unused, but save for posterity: private const string TwinGetTopic = "$iothub/twin/GET/?$rid={0}";
        ...
        private static readonly TimeSpan s_semaphoreSlimWaitIndefinite = TimeSpan.FromMilliseconds(-1);
        ...
        public TimeSpan TwinTimeout { get; set; } = TimeSpan.FromSeconds(60);

        #region Client operations

        public override async Task OpenAsync(TimeoutHelper timeoutHelper)
        {
            using var cts = new CancellationTokenSource(timeoutHelper.RemainingTime());
        ...
        public override async Task<Message> ReceiveAsync(CancellationToken cancellationToken)
        {
            ...
            bool hasMessage = await ReceiveMessageArrivalAsync(s_semaphoreSlimWaitIndefinite, cancellationToken).ConfigureAwait(true);
            return ProcessMessage(null, hasMessage);
        }
        ...
        public override async Task<Message> ReceiveAsync(TimeoutHelper timeoutHelper)
        {
            ...
            Message message = ProcessMessage(null, hasMessage);
        ...
        private async Task HandleIncomingTwinPatchAsync(Message message)
        {
            try
            {
                ...
                {
                    using var reader = new StreamReader(message.GetBodyStream(), Encoding.UTF8);
        ...
        private async Task HandleIncomingMethodPostAsync(Message message)
        {
            try
            {
                ...
                await _messageListener(mr).ConfigureAwait(true);
            }
            finally
            {
                message.Dispose();
...
#pragma warning disable CA2000 // Dispose objects before losing scope - will be disposed by user
...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        private async Task<Message> SendTwinRequestAsync(Message request, string rid, CancellationToken cancellationToken)
        {
            ...
            // Coordinates waiting for and then handling the response to our request, which will come back on another channel.
            using var responseReceived = new SemaphoreSlim(0);

            Message response = null;
            ExceptionDispatchInfo responseException = null;

            // To see the result of our request, subscribe to a callack that will *eventually*, hopefully fire
        ...
        public override async Task<Twin> SendTwinGetAsync(CancellationToken cancellationToken)
        {
            ...
            using var reader = new StreamReader(response.GetBodyStream(), Encoding.UTF8);
            string body = reader.ReadToEnd();

            try
            {
                TwinProperties props = JsonConvert.DeserializeObject<TwinProperties>(body);
