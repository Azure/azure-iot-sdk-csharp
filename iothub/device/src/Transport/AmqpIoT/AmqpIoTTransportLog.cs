...
namespace Microsoft.Azure.Devices.Client.Transport.AmqpIoT
{
    internal class AmqpIoTTransportLog : AmqpTrace
    {
        ...
        {
            if (Logging.IsEnabled) Logging.Info(exception, "AmqpAbortThrowingException:", "AmqpTransportLog");
        ...
        {
            if (Logging.IsEnabled) Logging.Associate(source, session, "AmqpAddSession:", $"{" source:"}{source}.{" session:"}{session}.{" localChannel:"}.{localChannel}.{" remoteChannel:"}.{remoteChannel}");
        ...
        {
            if (Logging.IsEnabled) Logging.Associate(session, link, "AmqpAttachLink", $"{" session:"}{session}.{" link:"}.{link}.{" localHandle:"}.{localHandle}.{" remoteHandle:"}.{remoteHandle}.{" linkName:"}.{linkName}.{" role:"}.{role}.{" source:"}.{source}.{" target:"}.{target}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpCacheMessage:", $"{" source:"}{source}.{" deliveryId:"}.{deliveryId}.{" count:"}.{count}.{" isPrefecthingBySize:"}.{isPrefecthingBySize}.{" totalCacheSizeInBytes:"}.{totalCacheSizeInBytes}.{" totalLinkCredit:"}.{totalLinkCredit}.{" linkCredit:"}.{linkCredit}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(connection, "AmqpCloseConnection:", $"{" source:"}{source}.{" connection:"}.{connection}.{" abort:"}.{abort}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpDeliveryNotFound:", $"{" source:"}{source}.{" deliveryTag:"}.{deliveryTag}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpDispose:", $"{" source:"}{source}.{" deliveryId:"}.{deliveryId}.{" settled:"}.{settled}.{" state:"}.{state}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpDynamicBufferSizeChange:", $"{" source:"}{source}.{" type:"}.{type}.{" oldSize:"}.{oldSize}.{" newSize:"}.{newSize}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(exception, "AmqpHandleException:", $"{" exception:"}{exception}.{" traceInfo:"}.{traceInfo}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(transport, "AmqpInsecureTransport:", $"{" source:"}{source}.{" transport:"}.{transport}.{" isSecure:"}.{isSecure}.{" isAuthenticated:"}.{isAuthenticated}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(ioEvent, "AmqpIoEvent:", $"{" source:"}{source}.{" ioEvent:"}.{ioEvent}.{" queueSize:"}.{queueSize}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpLinkDetach", $"{" source:"}{source}.{" name:"}.{name}.{" handle:"}.{handle}.{" action:"}.{action}.{" error:"}.{error}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpListenSocketAcceptError:", $"{" source:"}{source}.{" willRetry:"}.{willRetry}.{" error:"}.{error}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpLogError:", $"{" source:"}{source}.{" operation:"}.{operation}.{" message:"}.{message}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpLogOperationInformational:", $"{" source:"}{source}.{" operation:"}.{operation}.{" detail:"}.{detail}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpLogOperationVerbose:", $"{" source:"}{source}.{" operation:"}.{operation}.{" detail:"}.{detail}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpMissingHandle:", $"{" source:"}{source}.{" type:"}.{type}.{" handle:"}.{handle}");
            base.AmqpMissingHandle(source, type, handle);
        }

        public override void AmqpOpenConnection(object source, object connection)
        {
            if (Logging.IsEnabled) Logging.Info(connection, "AmqpOpenConnection:", $"{" source:"}{source}.{" connection:"}.{connection}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(obj, "AmqpOpenEntityFailed:", $"{" source:"}{source}.{" obj:"}.{obj}.{" name:"}.{name}.{" entityName:"}.{entityName}.{" error:"}.{error}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(obj, "AmqpOpenEntitySucceeded:", $"{" source:"}{source}.{" obj:"}.{obj}.{" entityName:"}.{entityName}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpReceiveMessage:", $"{" source:"}{source}.{" deliveryId:"}.{deliveryId}.{" transferCount:"}.{transferCount}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(link, "AmqpRemoveLink", $"{" connection:"}{connection}.{" session:"}.{session}.{" link"}.{link}.{" localHandle:"}.{localHandle}.{" remoteHandle:"}.{remoteHandle}.{" linkName:"}.{linkName}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(session, "AmqpRemoveSession:", $"{" source:"}{source}.{" session:"}.{session}.{" localChannel:"}.{localChannel}.{" remoteChannel:"}.{remoteChannel}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpSessionWindowClosed:", $"{" source:"}{source}.{" nextId:"}.{nextId}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpStateTransition:", $"{" source:"}{source}.{" operation:"}.{operation}.{" fromState:"}.{fromState}.{" toState:"}.{toState}");
        ...
        {
            if (Logging.IsEnabled) Logging.Info(source, "AmqpUpgradeTransport:", $"{" source:"}{source}.{" from:"}.{from}.{" to:"}.{to}");
