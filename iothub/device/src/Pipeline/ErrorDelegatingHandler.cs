        ...
        private async Task<T> ExecuteWithErrorHandlingAsync<T>(Func<Task<T>> asyncOperation)
        {
            try
            {
                ...
                catch (Exception ex) when (!Fx.IsFatal(ex))
                {
                    ...
                    else if (ex is IotHubClientException hubEx && hubEx.ErrorCode is IotHubErrorCode.NetworkErrors)
                    {
                        throw;
                    }
                    else if (IsNetworkExceptionChain(ex))
                    {
                        throw new IotHubClientException("Transient network error occurred, please retry.", IotHubErrorCode.NetworkErrors, ex);
                    }
                    else
                    {
                        throw;
