...
namespace Microsoft.Azure.Devices.E2ETests
{
    ...
    public class PlugAndPlayTests
    {
        ...
        {
            await CreateDeviceAndRetrieveTwinAsync(Client.TransportType.Mqtt_Tcp_Only, TestDeviceType.Sasl).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task DeviceTwin_Sas_Contains_ModelId_ConnectOver_Mqtt_Ws()
        {
            await CreateDeviceAndRetrieveTwinAsync(Client.TransportType.Mqtt_WebSocket_Only, TestDeviceType.Sasl).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task DeviceTwin_X509_Contains_ModelId_ConnectOver_Mqtt_Tcp()
        {
            await CreateDeviceAndRetrieveTwinAsync(Client.TransportType.Mqtt_Tcp_Only, TestDeviceType.X509).ConfigureAwait(false);
        }
        ...
        {
            await CreateDeviceAndRetrieveTwinAsync(Client.TransportType.Amqp_Tcp_Only, TestDeviceType.Sasl).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task DeviceTwin_Sas_Contains_ModelId_ConnectOver_Amqp_Ws()
        {
            await CreateDeviceAndRetrieveTwinAsync(Client.TransportType.Amqp_WebSocket_Only, TestDeviceType.Sasl).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task DeviceTwin_X509_Contains_ModelId_ConnectOver_Amqp_Tcp()
        {
            await CreateDeviceAndRetrieveTwinAsync(Client.TransportType.Amqp_Tcp_Only, TestDeviceType.X509).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task DeviceTwin_X509_Contains_ModelId_ConnectOver_Amqp_Ws()
        {
            await CreateDeviceAndRetrieveTwinAsync(Client.TransportType.Amqp_WebSocket_Only, TestDeviceType.X509).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task ModuleTwin_Contains_ModelId_ConnectOver_Mqtt_Tcp()
        {
            await CreateModuleAndRetrieveTwinAsync(Client.TransportType.Mqtt_Tcp_Only).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task ModuleTwin_Contains_ModelId_ConnectOver_Mqtt_Ws()
        {
            await CreateModuleAndRetrieveTwinAsync(Client.TransportType.Mqtt_WebSocket_Only).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task ModuleTwin_Contains_ModelId_ConnectOver_Amqp_Tcp()
        {
            await CreateModuleAndRetrieveTwinAsync(Client.TransportType.Amqp_Tcp_Only).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task ModuleTwin_Contains_ModelId_ConnectOver_Amqp_Ws()
        {
            await CreateModuleAndRetrieveTwinAsync(Client.TransportType.Amqp_WebSocket_Only).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task MultiplexedDevices_DeviceSak_HaveUniqueModelId_ConnectOver_Amqp_Tcp()
        {
            await CreateMultiplexedDevicesAndRetrieveTwinAsync(Client.TransportType.Amqp_Tcp_Only, ConnectionStringAuthScope.Device).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task MultiplexedDevices_DeviceSak_HaveUniqueModelId_ConnectOver_Amqp_Ws()
        {
            await CreateMultiplexedDevicesAndRetrieveTwinAsync(Client.TransportType.Amqp_WebSocket_Only, ConnectionStringAuthScope.Device).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task MultiplexedDevices_IotHubSak_HaveUniqueModelId_ConnectOver_Amqp_Tcp()
        {
            await CreateMultiplexedDevicesAndRetrieveTwinAsync(Client.TransportType.Amqp_Tcp_Only, ConnectionStringAuthScope.IoTHub).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task MultiplexedDevices_IotHubSak_HaveUniqueModelId_ConnectOver_Amqp_Ws()
        {
            await CreateMultiplexedDevicesAndRetrieveTwinAsync(Client.TransportType.Amqp_WebSocket_Only, ConnectionStringAuthScope.IoTHub).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task MultiplexedModules_HaveUniqueModelId_ConnectOver_Amqp_Tcp()
        {
            await CreateMultiplexedModulesAndRetrieveTwinAsync(Client.TransportType.Amqp_Tcp_Only).ConfigureAwait(false);
        }

        [TestMethod]
        public async Task MultiplexedModules_HaveUniqueModelId_ConnectOver_Amqp_Ws()
        {
            await CreateMultiplexedModulesAndRetrieveTwinAsync(Client.TransportType.Amqp_WebSocket_Only).ConfigureAwait(false);
        }

        private static async Task CreateMultiplexedDevicesAndRetrieveTwinAsync(Client.TransportType transportType, ConnectionStringAuthScope authScope)
        {
            // Setup
        ...
        private static async Task CreateMultiplexedModulesAndRetrieveTwinAsync(Client.TransportType transportType)
        {
            // Setup
            ...
            for (int i = 0; i < PoolingOverAmqp.MultipleConnections_DevicesCount; i++)
            {
                ...
                modules.Add(testModule, CreateModuleClientWithModelId(testModule, transportSettings, modelId));
            }
        ...
        private static ModuleClient CreateModuleClientWithModelId(TestModule testModule, ITransportSettings[] transportSettings, string modelId)
        {
            ...
            s_log.WriteLine($"{nameof(PlugAndPlayTests)}: Created module: deviceId={testModule.DeviceId}, moduleId={testModule.Id}, modelId={modelId}");

            return moduleClient;
        }

        private static ModuleClient CreateModuleClientWithModelId(TestModule testModule, Client.TransportType transportType, string modelId)
        {
            // Send model ID while initializing the module client, to mark the module as plug and play compatible.
            var options = new ClientOptions
            {
            ...
            s_log.WriteLine($"{nameof(PlugAndPlayTests)}: Created module: deviceId={testModule.DeviceId}, moduleId={testModule.Id}, modelId={modelId}, transportType={transportType}");

            return moduleClient;
        }

        private static async Task CreateDeviceAndRetrieveTwinAsync(Client.TransportType transportType, TestDeviceType deviceType)
        {
            // Setup

            // Create a device.
            TestDevice testDevice = await TestDevice.GetTestDeviceAsync(DevicePrefix, deviceType).ConfigureAwait(false);
            ...
            s_log.WriteLine($"{nameof(PlugAndPlayTests)}: Created device: deviceId={testDevice.Id}, modelId={options.ModelId}, auth={testDevice.Device.Authentication}, transport={transportType}");
        ...
        private static async Task CreateModuleAndRetrieveTwinAsync(Client.TransportType transportType)
        {
            // Setup
            ...
            using ModuleClient moduleClient = CreateModuleClientWithModelId(testModule, transportType, TestModelId);
