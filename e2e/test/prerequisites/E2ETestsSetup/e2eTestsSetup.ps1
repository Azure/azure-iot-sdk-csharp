...
Get-ChildItem "Cert:\LocalMachine\My" | Where-Object { $_.Issuer.Contains("CN=iothubx509device1") } | Remove-Item
Get-ChildItem .\ | Where-Object { $_.Name.EndsWith(".pfx") } | Remove-Item
Get-ChildItem .\ | Where-Object { $_.Name.EndsWith(".cer") } | Remove-Item
Get-ChildItem .\ | Where-Object { $_.Name.EndsWith(".p7b") } | Remove-Item
...
if ($isVerified -eq 'false') {
    ...
    $verificationCertArgs = @{"-DnsName" = $requestedCommonName;
        "-CertStoreLocation"             = "cert:\LocalMachine\My";
        "-NotAfter"                      = (get-date).AddYears(2);
...
az iot dps enrollment create -g $ResourceGroup --dps-name $deviceProvisioningServiceName --enrollment-id $individualEnrollmentId --device-id $individualDeviceId --attestation-type x509 --certificate-path $individualDeviceCertPath --output none
...
while ($retry -eq $true) {
    try {
        Write-Host "`nGranting $appId Reader role assignment to the $Resourcegroup resource group - Trial $retryCount"
...
az ad app credential reset --id $appId --create-cert --keyvault $ResourceGroup --cert $ResourceGroup --output none
Write-Host "`nSuccessfully created a self signed certificate for your application $appRegistrationName in $ResourceGroup Keyvault: Cert name : $ResourceGroup";
...
az keyvault secret download --file .\selfSignedCert --vault-name $ResourceGroup -n $ResourceGroup --encoding base64
...
$secretsKeyVaultName = "Env$ResourceGroup"
if ($secretsKeyVaultName.Length -ge 24) {
    $secretsKeyVaultName = $secretsKeyVaultName.Substring(0, 24)
}
$keyVaultExists = az keyvault list -g $ResourceGroup --query "[?name=='$secretsKeyVaultName']" --output tsv
if (-not $keyVaultExists) {
    Write-Host("`nCreating KeyVault $secretsKeyVaultName")
    az keyvault create -g $ResourceGroup --name $secretsKeyVaultName --output none
}
Write-Host("`nWriting secrets to KeyVault $secretsKeyVaultName")
...
az keyvault secret set --vault-name $secretsKeyVaultName --name "CUSTOM-ALLOCATION-POLICY-WEBHOOK" --value $CustomAllocationPolicyWebHook --output none
...
############################################################################################################################
# Cleanup certs and files created by the script
############################################################################################################################
Write-Host("`nCleanup certificates and files created by the script")
Get-ChildItem "Cert:\LocalMachine\My" | Where-Object { $_.Issuer.Contains("CN=$subjectPrefix") } | Remove-Item
Get-ChildItem "Cert:\LocalMachine\My" | Where-Object { $_.Issuer.Contains("CN=xdevice1") } | Remove-Item
Get-ChildItem "Cert:\LocalMachine\My" | Where-Object { $_.Issuer.Contains("CN=iothubx509device1") } | Remove-Item
Get-ChildItem .\ | Where-Object { $_.Name.EndsWith(".pfx") } | Remove-Item
...
Write-Host "`nOutput in '$currentDir\iotPowershellConfig.ps1'"
$file = New-Item  -Path $currentDir -Name "iotPowershellConfig.ps1" -ItemType "file" -Force
Add-Content -Path $file.PSPath -Value "Invoke-Expression `".\LoadEnvironmentVariablesFromKeyVault.ps1 -SubscriptionId $SubscriptionId -KeyVaultName $secretsKeyVaultName`""

############################################################################################################################
# Configure Environment Variables
############################################################################################################################
Invoke-Expression ".\iotPowershellConfig.ps1"

$endTime = (Get-Date)
$ElapsedTime = (($endTime - $startTime).TotalMinutes)
Write-Host "`n`nCompleted in $ElapsedTime minutes. Run the generated file - iotPowershellConfig.ps1 to load Environment variables on future runs. Values will be overwritten whenever you run e2eTestsSetup.ps1`n"
