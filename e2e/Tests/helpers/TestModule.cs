...
namespace Microsoft.Azure.Devices.E2ETests.Helpers
{
    internal class TestModule : IAsyncDisposable
    {
        ...
        /// Factory method.
        /// </summary>
        internal static async Task<TestModule> GetTestModuleAsync(string deviceNamePrefix, string moduleNamePrefix, CancellationToken ct)
        {
            var testModule = new TestModule
            {
                TestDevice = await TestDevice.GetTestDeviceAsync(deviceNamePrefix, ct: ct).ConfigureAwait(false)
            };
            ...
            testModule.Module = await sc.Modules.CreateAsync(new Module(deviceName, moduleName), ct).ConfigureAwait(false);
