...
call nuget restore -config "%current-path%\NuGet.Config" "%build-root%\device\iothub_csharp_deviceclient.sln"
if %build-clean%==1 (
    call :clean-a-solution "%build-root%\device\iothub_csharp_deviceclient.sln" %build-config% %build-platform%
    if not !errorlevel!==0 exit /b !errorlevel!
)
call :build-a-solution "%build-root%\device\iothub_csharp_deviceclient.sln" %build-config% %build-platform%
if not !errorlevel!==0 exit /b !errorlevel!
