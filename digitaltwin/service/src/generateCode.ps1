...
try {
	...
	#Edit the protocol layer models to all be internal
	Get-ChildItem ./Generated/Models *.cs -recurse |
		Foreach-Object {
			$modelClassCode = ($_ | Get-Content) 
			$modelClassCode = $modelClassCode -replace 'public partial class', 'internal partial class'
			[IO.File]::WriteAllText($_.FullName, ($modelClassCode -join "`r`n"))
		}
