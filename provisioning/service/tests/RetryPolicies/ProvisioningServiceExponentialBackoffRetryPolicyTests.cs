...
namespace Microsoft.Azure.Devices.Provisioning.Service.Tests
{
    [TestClass]
    [TestCategory("Unit")]
    public class ProvisioningServiceExponentialBackoffRetryPolicyTests
    {
        [TestMethod]
        public void ExponentialBackoffRetryPolicy_DoesNotUnderflowDelay()
        {
            ...
            var exponentialBackoff = new ProvisioningServiceExponentialBackoffRetryPolicy(MaxRetryAttempts, TimeSpan.FromDays(365), false);
            TimeSpan previousDelay = TimeSpan.Zero;

            for (uint retryCount = 1; retryCount < MaxRetryAttempts; retryCount++)
            {
                // act
                exponentialBackoff.ShouldRetry(retryCount, new ProvisioningServiceException("", true), out TimeSpan delay).Should().BeTrue();
        ...
        {
            // arrange
            var exponentialBackoff = new ProvisioningServiceExponentialBackoffRetryPolicy(uint.MaxValue, TimeSpan.FromDays(30), false);
            TimeSpan previousDelay = TimeSpan.Zero;
            uint exponent = retryCount + 6; // starts at 7
            // act
            exponentialBackoff.ShouldRetry(retryCount, new ProvisioningServiceException("", true), out TimeSpan delay);
