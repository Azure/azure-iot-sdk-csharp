    ...
    internal sealed class ProvisioningTransportHandlerAmqp : ProvisioningTransportHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                amqpMessage = payload == null
                    ...
                    : AmqpMessage.Create(new MemoryStream(JsonSerializer.SerializeToUtf8Bytes(payload, JsonSerializerSettings.Options)), true);
                ...
                string jsonResponse = await streamReader
                    ...
                    .ConfigureAwait(false);
                RegistrationOperationStatus status = JsonSerializer.Deserialize<RegistrationOperationStatus>(jsonResponse, JsonSerializerSettings.Options);
        ...
        internal void ValidateOutcome(Outcome outcome)
        {
            if (outcome is Rejected rejected)
            {
                try
                {
                    ProvisioningErrorDetailsAmqp errorDetails = JsonSerializer.Deserialize<ProvisioningErrorDetailsAmqp>(rejected.Error.Description, JsonSerializerSettings.Options);
