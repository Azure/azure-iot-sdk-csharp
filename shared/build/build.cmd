...
rem -----------------------------------------------------------------------------
rem -- build csharp iot shared
rem -----------------------------------------------------------------------------

call nuget restore -config "%current-path%\NuGet.Config" "%build-root%\shared\iothub_csharp_shared.sln"
if %build-clean%==1 (
    call :clean-a-solution "%build-root%\shared\iothub_csharp_shared.sln" %build-config% %build-platform%
    if not !errorlevel!==0 exit /b !errorlevel!
)
call :build-a-solution "%build-root%\shared\iothub_csharp_shared.sln" %build-config% %build-platform%
if not !errorlevel!==0 exit /b !errorlevel!
